...
matrix* solve_ode(
	matrix(*diff)(double, matrix, matrix, matrix), 
	double t0, // czas poczatkowy
	double dt,  // krok czasowy
	double tend, // czas koncowy
	matrix Y0, // zmienne poczatkowe
	matrix ud1, 
	matrix ud2
){
	try
	{
		...
		// iloœæ wierszy, tj. liczba elementów w wektorze Y
		int n = s[ 0 ];
		delete[]s;

		// macierz zawieraj¹ca wyniki ka¿dego z N kroków
		// S[0] - wartoœæ parametru steruj¹cego t w danym kroku
		// S[1] - rozwi¹zania równañ ró¿niczkowych, tj. stan zmiennych w wektorze Y dla danego t
		...
		// obliczenie wartoœci funkcji w danych krokach metod¹ RK4
